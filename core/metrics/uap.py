"""
Copied and Modified from Facebook DISC code
...
def precision_recall(
    ...
    precisions, recalls, thresholds
        ordered by increasing recall, as for a precision-recall_at_threshold curve
    """
...
def average_precision(recalls: np.ndarray, precisions: np.ndarray):
    ...
    ----------
    recalls : np.ndarray
        Recalls. Must be sorted by increasing recall_at_threshold, as in a PR curve.
    precisions : np.ndarray
        Precisions for each recall_at_threshold value.
...
def uap(results: Union[List[Tuple[str, List[Tuple[str, float]]]], Dict[str, List[Tuple[str, float]]]],
    ...
    p, r, t = precision_recall(labels, scores, pair_gt_cnt)

    # Micro-average precision
    ap = average_precision(r, p)
    details = dict(p=p, r=r, t=t)
    return ap, details
...
def evaluate_metrics(submission_df: pd.DataFrame, gt_df: pd.DataFrame):
    ...
    print("recall_at_threshold {} {} {}".format(sum(y_true), len(gt_pairs), sum(y_true) / len(gt_pairs)))
