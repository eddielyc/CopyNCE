...
from core.models import (
    build_copy_descriptor_for_eval,
...
def inference(cfg, query_feats, reference_feats, training_feats=None):
    ...
    if training_feats is not None:
        assert isinstance(training_feats, np.ndarray)
        training_norms = np.linalg.norm(training_feats, axis=1, keepdims=True)
        training_feats = training_feats / training_norms
        codec = faiss.index_factory(dim, f"PCAW{dim},L2norm,Flat")
        codec.train(training_feats)

        query_feats = codec.sa_encode(query_feats)
        query_feats = np.frombuffer(query_feats, dtype=np.float32).reshape(-1, dim)

        reference_feats = codec.sa_encode(reference_feats)
        reference_feats = np.frombuffer(reference_feats, dtype=np.float32).reshape(-1, dim)
    ...
    topk_cosines, topk_indices = index.search(query_feats, search_rank)
    ...
    for i in tqdm(range(query_feats.shape[0])):
        _scores = [
            query_ids[i],
            [[uuid, float(s)] for uuid, s in zip(reference_ids[topk_indices[i].tolist()], topk_cosines[i])]
        ]
...
def eval_des(model, cfg, epoch=None):
    ...
    que_path = output_dir / f'query_{key}_ep-{epoch}.pth'
    ref_path = output_dir / f'reference_{key}_ep-{epoch}.pth'
    
    # extract or load features
    if (not que_path.exists()) or (not ref_path.exists()):
        _, _, que_data_loader, ref_data_loader = build_data_for_descriptor_eval(cfg)
        outputs_que = extract_features_with_dataloader(
            model,
            que_data_loader,
        ...
        if distributed.is_main_process():
            query_feats = outputs_que[key]
            torch.save(query_feats, que_path)
            logger.info(f"EVAL -- Query features dumped.")
        ...
        torch.cuda.synchronize()
        if distributed.is_main_process():
            reference_feats = outputs_ref[key]
            torch.save(reference_feats, ref_path)
            logger.info(f"EVAL -- Reference feature dumped.")
    else:
        if distributed.is_main_process():
...
def main():
    ...
    model, epoch = build_copy_descriptor_for_eval(cfg)
    # model, epoch = build_sscd_resnet50_descriptor_for_eval(cfg)
    # model, epoch = build_resnet50_descriptor_for_eval(cfg)
