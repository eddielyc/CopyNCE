...
class _TorchDistributedEnvironment:
    def __init__(self):
        ...
        if all([key in os.environ for key in ["RANK", "WORLD_SIZE", "LOCAL_RANK", "LOCAL_WORLD_SIZE", "MASTER_PORT"]]):
            self._set_from_single_node()
        else:
            self._set_in_debug_mode()
            logger.info(f"TorchDistributedEnvironment is enabled in debug mode.")
        # raise RuntimeError("Can't initialize PyTorch distributed environment")

    # Single node and GPU job (i.e. local script run)
    def _set_from_single_node(self):
    ...
    def _set_in_debug_mode(self):
        logger.info("Initialization in debug mode")
        self.master_addr = "127.0.0.1"
    ...
    def export(self, *, overwrite: bool) -> "_TorchDistributedEnvironment":
        ...
        logger.info(f"Distributed envs: {env_vars}")
