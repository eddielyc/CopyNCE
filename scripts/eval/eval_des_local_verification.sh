...
print_usage() {
    echo "Usage: eval_des_local_verification.sh [option1] [option2] [option3]"
    ...
    echo "   option2 - EVAL_CONFIG: path of eval config file, the code will load config from 'core/configs/eval/descriptor/{EVAL_CONFIG}'."
    echo "   option3 - MODE: search mode, default is '1:N'."
}
...
# set EVAL_CONFIG
EVAL_CONFIG="core/configs/eval/descriptor/$2"
...
torchrun -m \
  ...
  --nproc_per_node=8 \
  --master_port $PORT \
  core.run.eval.eval_des_local_verification \
